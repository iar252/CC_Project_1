...
function draw() {
 if (scrapbook == true){
  ...
  if(didArrow==true && lastArrow == false){
    scrapbook = false;
    background(186, 158, 50);
    nextPage();
...
function keyPressed(){ // this function here is used to keep track of the controls. 
  // went over during office hours
  // example: scrapbook was initially true, so when the computer sees the condition on like 47,
  // it continues to execute the block of code. on line 52, scrapbook is now false so that
  // the previous graphics no longer display, but instead display nextPage();
